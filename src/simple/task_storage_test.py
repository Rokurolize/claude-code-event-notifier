...
from task_storage_improved import TaskStorage, SimpleLock, TaskInfo
...
class TestTaskStorageImproved(unittest.TestCase):
        ...
        def worker(worker_id):
            try:
                success = TaskStorage.track_task_start(
                    "session1",
                    f"task_{worker_id}",
                    {"status": "started", "worker": worker_id}
                )
    ...
    def test_cleanup_old_sessions(self):
        ...
        test_data = {
            "old_session": {
                "task1": {"start_time": old_time, "status": "completed"}
            },
            "new_session": {
                "task1": {"start_time": new_time, "status": "started"}
            },
            "empty_session": {}
        }
