...
class HTTPRequest(Generic[T]):
    ...
    def __init__(
        self, url: str, method: str, data: T, headers: dict[str, str] | None = None
    ):
        self.url = url
        self.method = method
        self.data = data
...
class HTTPResponse(Generic[T]):
    ...
    def __init__(
        self, status_code: int, data: T, headers: dict[str, str] | None = None
    ):
...
def is_valid_event_data(obj: Any) -> TypeGuard[BaseEventData]:
    ...
    return (
        ...
        and "transcript_path" in obj
        and "hook_event_name" in obj
    )
