...
def run_notifier_event(event_type: str, event_data: dict, script_path: Path) -> tuple[bool, str]:
...
def test_thread_creation():
    ...
    for i, event in enumerate(test_events, 1):
        ...
        print(f"[{i}/{len(test_events)}] Sending {event_type} event...", end="", flush=True)
        ...
        if success:
            ...
            if "Thread" in output or "thread" in output:
                print(f"    💬 Thread info: {output.split('Thread')[0]}Thread{output.split('Thread')[1].split(chr(10))[0]}")
        else:
            print(" ❌")
    ...
    success, output = run_notifier_event("Notification", second_event["data"], script_path)
    ...
    print(f"5. Expected thread names:")
    ...
    if all_success:
        ...
        print("🧵 Thread creation test completed. Check Discord to verify threading behavior.")
    else:
