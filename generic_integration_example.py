...
class BashCompatibleValidator(LegacyCompatibleValidator[dict[str, Any]]):
    ...
    def validate_legacy(self, data: dict[str, Any]) -> bool:
        ...
        return (
            ...
            "command" in data and
            isinstance(data["command"], str) and
            bool(data["command"].strip())
        )
    
    def validate_typed(self, data: dict[str, Any]) -> bool:
        """Validate typed bash tool input."""
        return (
            self.validate_legacy(data) and
    ...
    def _validate_optional_fields(self, data: dict[str, Any]) -> bool:
        ...
        if "timeout" in data and not isinstance(data["timeout"], (int, float)):
            return False
        if "working_directory" in data and not isinstance(data["working_directory"], str):
            return False
        return True
...
class FormatterRegistry(Generic[TEventData]):
    ...
    def __init__(self) -> None:
        self._typed_formatters: dict[str, Callable[[TEventData, str], dict[str, Any]]] = {}
        self._legacy_formatters: dict[str, Callable[[dict[str, Any], str], dict[str, Any]]] = {}
    
    def register_typed(
        self, 
        event_type: str, 
        formatter: Callable[[TEventData, str], dict[str, Any]]
    ) -> None:
    ...
    def get_formatter(self, event_type: str) -> Callable[[Any, str], dict[str, Any]]:
        ...
        elif event_type in self._legacy_formatters:
...
def format_bash_event_generic(
    event_data: EnhancedEventData[dict[str, Any]], 
    session_id: str
) -> dict[str, Any]:
    ...
    if "tool_response" in event_data:
        ...
        if isinstance(response, dict):
            ...
            embed["fields"].extend([
                ...
                {"name": "Output", "value": f"```\n{stdout[:500]}\n```" if stdout else "No output", "inline": False}
            ])
            
            if stderr:
                embed["fields"].append({"name": "Errors", "value": f"```\n{stderr[:500]}\n```", "inline": False})
...
class GenericEventProcessor(Generic[TEventData]):
    ...
    def register_validator(self, tool_name: str, validator: LegacyCompatibleValidator[Any]) -> None:
...
def migrate_legacy_tool_input(legacy_input: dict[str, Any], tool_name: str) -> EnhancedToolInput[dict[str, Any]]:
...
def migrate_legacy_event_data(legacy_event: dict[str, Any]) -> EnhancedEventData[dict[str, Any]]:
...
def demonstrate_integration() -> None:
    ...
    processor: GenericEventProcessor[EnhancedEventData[dict[str, Any]]] = GenericEventProcessor()
    ...
    processor.add_processor(lambda data: {
        **data,
    ...
    # Migration demo
    migrated_data = validator.migrate_to_typed({"command": "pwd", "description": "Get current directory"})
...
class CompatibilityLayer:
    ...
    def wrap_legacy_formatter(
        ...
        def wrapper(event_data: EnhancedEventData[dict[str, Any]], session_id: str) -> dict[str, Any]:
    ...
    def is_legacy_format(event_data: dict[str, Any]) -> bool:
        ...
        return (
            "tool_input" in event_data and
            isinstance(event_data["tool_input"], dict) and
            "typed_data" not in event_data["tool_input"]
        )
