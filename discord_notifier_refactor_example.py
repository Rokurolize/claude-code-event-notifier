...
class HTTPResult(Generic[T]):
    ...
    def __init__(self, success: bool, data: T, status_code: int, error: str | None = None):
...
class GenericHTTPClient(Generic[TConfig]):
    ...
    def post_bot_api(self, url: str, data: DiscordMessage, token: str) -> HTTPResult[DiscordMessage]:
...
class TypedDiscordNotifier:
    ...
    def _register_formatters(self) -> None:
        ...
        notification_formatter = GenericFormatter[dict[str, Any]](self._format_notification)
        self.formatter_registry.register('Notification', notification_formatter)
    
    def _format_pre_tool_use(self, event_data: dict[str, Any], session_id: str) -> DiscordEmbed:
    ...
    def _format_post_tool_use(self, event_data: dict[str, Any], session_id: str) -> DiscordEmbed:
    ...
    def _format_notification(self, event_data: dict[str, Any], session_id: str) -> DiscordEmbed:
    ...
    def _send_message(self, message: DiscordMessage) -> bool:
        ...
        if self.config['bot_token'] and self.config['channel_id']:
            ...
            result = self.http_client.post_bot_api(url, message, self.config['bot_token'])
            if result.success:
...
def demonstrate_refactored_notifier():
    ...
    notification_success = notifier.process_event('Notification', notification_event_data)
