...
def run_discord_event(event_type: str, event_data: dict, script_path: Path) -> tuple[bool, str]:
...
def test_thread_functionality():
    ...
    if results["test_1"]["thread_info"]["thread_created"]:
        expected_threads.append(f"Session {session_1[:8]} - ID: {results['test_1']['thread_info']['thread_id']}")
    
    if results["test_3"]["thread_info"]["thread_created"]:
        expected_threads.append(f"Session {session_2[:8]} - ID: {results['test_3']['thread_info']['thread_id']}")
